...
namespace Common
{
	...
	private:
		std::once_flag s_curl_was_initialized;
		...
		size_t size, AllowedReturnCodes codes)
	{
		...
		if (res != CURLE_OK)
		{
			ERROR_LOG(COMMON, "Failed to {} {}: {}", type, url, m_error_string);
			return{};
		}
		...
		if (response_code != 200)
		{
			if (buffer.empty())
			{
				ERROR_LOG(COMMON, "Failed to {} {}: server replied with code {}", type, url,
					response_code);
			}
			else
			{
				ERROR_LOG(COMMON, "Failed to {} {}: server replied with code {} and body\n\x1b[0m{:.{}}",
